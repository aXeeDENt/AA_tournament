def strategy_round_2(opponent_id: int, my_history: dict[int, list[int]], opponents_history: dict[int, list[int]]) -> tuple[int, int]:
    """Enhanced strategy for Round 2 with opponent selection"""
    ...
    for opp_id in opponents_history:
        if len(my_history.get(opp_id, [])) >= 200: continue
        first_four = (opponents_history[opp_id][:4] + [1]*(4-len(opponents_history[opp_id][:4])))
        if first_four == [1,0,0,0]: categories['best'].append(opp_id)
        elif sum(first_four) == 0: categories['good'].append(opp_id)
        elif first_four == [0,1,0,0]: categories['normal'].append(opp_id)
        elif sum(first_four) == 4: categories['bad'].append(opp_id)
        elif first_four == [0,1,1,1]: categories['worst'].append(opp_id)
        else:
            neg_count = 4 - sum(first_four)
            if neg_count >= 3: categories['good'].append(opp_id)
            elif neg_count == 2: categories['normal'].append(opp_id)
            elif neg_count == 1: categories['bad'].append(opp_id)
